  ...
  function updateCartFigures() {
    ...
    request.onsuccess = function (event) {
      ...
      cartTotal = items.reduce((sum, item) => sum + ((item.price || 0) * (item.quantity || 1)), 0);
      document.getElementById("header-qty").textContent = cartCount;
      document.getElementById("header-price").textContent = `$${cartTotal.toLocaleString()}`;
    };
  }
  ...
  document.querySelectorAll(".product-item").forEach((item) => {
    ...
    const priceRaw = item.querySelector(".product-price")?.textContent?.trim() || "$0";
